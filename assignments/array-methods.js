...
let fullName = [];
runners.forEach(currentValue=>fullName.push(`${currentValue.first_name} ${currentValue.last_name}`));
...
runners.map(currentValue=> allCaps.push(currentValue.first_name.toUpperCase()));
...
runners.filter(function(element){
    if(element.shirt_size === 'L'){
        return largeShirts.push(element);
    }
})
...
let ticketPriceTotal = runners.reduce(function(acc, cur){
...
runners.forEach(function(currentValue){
    lastLog.push(currentValue.last_name);
  })
// Problem 2
let email =[];
runners.map(function(currentValue){
    email.push(currentValue.company_name.toUpperCase()); 
  })
